...
function Protected({ children, roles }) {
      ...
      if (!logged) {
        try {
          ...
          if (userData) {
            ...
            const businessId =
              userData.adminID || userData.officialID || userData.victimID || null;
            if (businessId)
              sessionStorage.setItem("actorBusinessId", String(businessId));
            logged = true;
          }
        } catch (e) {
  ...
  if (!authed) {
    ...
    return (
      <Navigate
        to={redirectToAdmin ? "/admin/login" : "/login"}
        replace
      />
    );
  }
