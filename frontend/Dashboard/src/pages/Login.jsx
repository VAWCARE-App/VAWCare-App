...
export default function Login() {
  ...
  const onFinish = async (values) => {
    try {
      ...
      // If the server indicates account is pending approval
      if (String(msg).toLowerCase().includes('pending') || String(msg).toLowerCase().includes('pending approval')) {
        notificationApi.info({
          ...
          description: (
            <div style={{ fontSize: 13, color: '#666' }}>
              ...
              <p style={{ margin: '8px 0 0 0', fontSize: 12, color: '#999' }}>
                Please wait for administrator approval to access your account.
              </p>
            </div>
          ),
          ...
          style: {
            borderRadius: 16,
            padding: '16px 20px',
            boxShadow: '0 12px 32px rgba(24, 144, 255, 0.2), 0 4px 12px rgba(24, 144, 255, 0.12)',
        ...
        // Update modal state but don't show it (notification is cooler)
