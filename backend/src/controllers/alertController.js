...
async function sendEmailsForAlert(alert, req) {
  try {
    ...
    const sampleTime = a.location?.timestamp ? new Date(a.location.timestamp).toLocaleString('en-US', { timeZone: 'Asia/Manila' }) : null;
  ...
  <p>Time: ${new Date(a.createdAt).toLocaleString('en-US', { timeZone: 'Asia/Manila' })}</p>
      <hr>
...
const sendSOSEmail = asyncHandler(async (req, res) => {
  try {
    ...
    const sampleTime = alert.location?.timestamp ? new Date(alert.location.timestamp).toLocaleString('en-US', { timeZone: 'Asia/Manila' }) : null;
    ...
    ${accuracy ? `<p>Reported accuracy: <b>${accuracy}</b></p>` : ''}
    ${sampleTime ? `<p>Sample time: <b>${sampleTime}</b></p>` : ''}
    <p>Time: ${new Date(alert.createdAt).toLocaleString('en-US', { timeZone: 'Asia/Manila' })}</p>
      <hr>
      <p>This message was generated by the VAWCare Barangay Support System.</p>
    `;
